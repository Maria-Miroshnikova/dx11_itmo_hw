...
RectangleComponent::RectangleComponent(Game* g) : GameComponent(g) {
	...
	DirectX::XMFLOAT4(-0.5f, -0.5f, 0.5f, 1.0f),	DirectX::XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f),
	DirectX::XMFLOAT4(0.5f, -0.5f, 0.5f, 1.0f),	DirectX::XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f),
	SimpleMath::Vector4(-0.5f, 0.5f, 0.5f, 1.0f),	SimpleMath::Vector4(.0f, 1.0f, .0f, 1.0f),
	};
	std::swap(points, pointsTmp);
	transform_data.size_c = SimpleMath::Vector4(1.0f, 1.0f, 1.0f, 1.0f);
}
...
void RectangleComponent::Initialize()
{
	...
	auto res = D3DCompileFromFile(L"./Shaders/PlatformShader.hlsl",
		nullptr /*macros*/,
		nullptr /*include*/,
		"VSMain",
		"vs_5_0",
	...
	if (FAILED(res)) {
		...
		{
			MessageBox(game->Display->hWnd, L"PlatformShader.hlsl", L"Missing Shader File", MB_OK);
		}
	...
	res = D3DCompileFromFile(L"./Shaders/PlatformShader.hlsl",
	...
	constBufDesc.ByteWidth = sizeof(offset_and_size);
...
{
	game->Context->UpdateSubresource(constBuffer, 0, 0, &transform_data, 0, 0);
}
...
{
	transform_data.off.x = pos.x;
	transform_data.off.y = pos.y;
}

void RectangleComponent::SetSizeCoeff(DirectX::SimpleMath::Vector2 size_c)
{
	transform_data.size_c.x = size_c.x;
	transform_data.size_c.y = size_c.y;
} 

DirectX::SimpleMath::Vector2 RectangleComponent::GetPosition() const
{
	return DirectX::SimpleMath::Vector2(transform_data.off.x, transform_data.off.y);
}